...
export default function ChapterSection({ chapterList, userEnrolledCourse }) {
  ...
  const checkIsChapterCompleted = (chapterId) => {
    ...
    return completedChapters.some(item => item.chapterId === chapterId);
  };

  return chapterList && (
    <View style={{ padding: 10, backgroundColor: Colors.WHITE, marginTop: 20, borderRadius: 15, marginBottom: 27 }}>
...
const styles = StyleSheet.create({
  ...
  CompleteChapter: {
    display: 'flex', flexDirection: 'row', backgroundColor: Colors.LIGHT_GREEN, alignItems: 'center', justifyContent: 'space-between',
    padding: 15, borderWidth: 1, borderRadius: 10, marginTop: 10, borderColor: Colors.GREEN
  }
});
